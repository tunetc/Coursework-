...
async def search_skins(query: str = ""):
    """Розумний токенізований пошук скінів у RAM за окремими словами."""
...
async def get_parsing_task():
    """Ендпоінт для Edge-воркерів: миттєво віддає задачу з голови черги за O(1)."""
...
async def check_cache(market_hash_name: str):
    """Перевіряє наявність свіжого запису в оперативній пам'яті сервера."""
...
async def process_client_data(payload: ClientPayload):
    """Приймає сирі ціни, оновлює RAM-кеш і планує повернення скіна в чергу лише після згасання TTL."""
    try:
...
async def get_all_valid_cache():
    """Повертає всі активні записи з кешу для зведених таблиць."""
    valid_records = []
    current_time = time.time()
